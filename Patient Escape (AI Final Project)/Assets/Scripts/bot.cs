...
public class Bot : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Initialize with a random position on the navmesh
        Vector3 randomPosition = GetRandomPositionOnNavMesh();
        agent.Warp(randomPosition);
    }

    // Method to get a random position on the navmesh
    Vector3 GetRandomPositionOnNavMesh()
    {
        NavMeshTriangulation navMeshData = NavMesh.CalculateTriangulation();
        int randomIndex = Random.Range(0, navMeshData.indices.Length - 3);
        Vector3 randomPosition = Vector3.Lerp(navMeshData.vertices[navMeshData.indices[randomIndex]], navMeshData.vertices[navMeshData.indices[randomIndex + 1]], Random.value);
    ...
    void Wander()
    {
        ...
        float wanderJitter = 5; // Try to change the wanderJitter between small and big values to see what happens with the movement
    ...
    void Update()
    {
        Debug.DrawRay(this.transform.position, this.transform.forward * 4, Color.blue);  // Forward direction of the bot

        // Visualize the direction to the target (line from the bot to the target)
        Debug.DrawLine(this.transform.position, target.transform.position, Color.green);  // Direction to target
        ...
        // By default, the bot will pursue the target using the advanced prediction in the Pursue method
        //Pursue();
